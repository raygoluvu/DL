...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if(stop - start < 10):
      ...
      x = torch.arange(start + (10 - start%10), stop, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    modified = torch.tensor([[0, 1, 2, 2, 2, 2], [0, 1, 2, 2, 2, 2], [3, 4, 3, 4, 5, 5], [3, 4, 3, 4, 5, 5]])
    x[:4, :6] = modified
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    C = 1 + max(x)
    y = torch.zeros(C, C, dtype=torch.float32)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    tmp = x.view(2, 3, 4)
    y = tmp.transpose(1, 0).contiguous().view(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigma = torch.sqrt(torch.sum((x - mu) ** 2, dim=0) / x.shape[0])
    y = ((x - mu)/sigma).round()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
